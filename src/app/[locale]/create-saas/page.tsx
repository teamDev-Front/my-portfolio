...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.createSaas' });

  const localPath = getPathname({ href: '/create-saas', locale: l });
    ...
    keywords:
      l === 'pt-BR'
        ? [
            ...
            'SaaS MVP',
            'agência SaaS Brasil',
            'desenvolver produto digital',
            'criar sistema multi-tenant',
            'plataforma com Stripe',
            'SaaS Jacareí',
            'desenvolvedor SaaS São José dos Campos',
            'fazer produto SaaS',
            'startup SaaS desenvolvimento',
          ]
        : [
            'build a SaaS',
            'SaaS development',
...
export default async function CreateSaasPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const l = locale as SupportedLocale;

  const tNav = await getTranslations({ locale, namespace: 'nav' });
  const tLand = await getTranslations({ locale, namespace: 'landing.createSaas' });
